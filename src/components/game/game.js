...
class Game extends React.Component {
	...
	loop = () => {
		...
		this.pathIndex = 0;
		this.nextFigureField = this.nextFigureField.map(row => row.map(cell => (cell = 0)));
		this.currentFigure = this.nextFigure;
		this.figurePath = this.currentFigure.path[0];
		this.drawFigure(this.field, this.figurePath, this.figureCoord.x, this.figureCoord.y);
		this.nextFigure = this.getRandomFigure();
	...
	rotateFigure() {
		...
		if (canRotate) {
			this.pathIndex = this.pathIndex >= this.currentFigure.path.length - 2 ? -1 : this.pathIndex + 1;
